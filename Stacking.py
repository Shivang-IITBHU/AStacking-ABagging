...
jvm.start() 
data = con.load_any_file("Train_Live_SpoofPP.arff") #to load the required file
data_copy = Instances.copy_instances(data)
test = con.load_any_file("Test_Live_SpoofPP.arff")
...
#test.delete_last_attribute()
#data_copy.delete_last_attribute()
validation = con.load_any_file("Valid_LivePP.arff")
...
class Instances(JavaObject):
	...
	print("----------------------------------------------------------------------------------")
	trees = [Classifier(classname="weka.classifiers.functions.VotedPerceptron" )for i in range(0, num_of_trees)]
	for i in range(0, num_of_clusters):
	...
	accuracy(trees, num_of_trees, test_copy)
	print("----------------------------------------------------------------------------------")
	trees = [Classifier(classname="weka.classifiers.trees.RandomForest" )for i in range(0, num_of_trees)]
	for i in range(0, num_of_clusters):
	...
	meta = MultipleClassifiersCombiner(classname="weka.classifiers.meta.Stacking", options=["-M", "weka.classifiers.functions.Logistic"] )
	classifiers = list()
